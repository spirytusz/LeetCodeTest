...
public class CourseSchedule {
    public static boolean canFinish(int numCourses, int[][] prerequisites) {
        ...
        for (int[] pair : prerequisites) {
            ...
            if (adjMatrix[prerequisite] == null) {
                adjMatrix[prerequisite] = new ArrayList<>();
            }
            adjMatrix[prerequisite].add(course);
            inDegrees[course]++;
        }

        return topologicalSort(adjMatrix, inDegrees);
    }

    private static boolean topologicalSort(List<Integer>[] adjMatrix, int[] inDegrees) {
        ...
        while (!queue.isEmpty()) {
            ...
            List<Integer> list = adjMatrix[current];
            if (list != null) {
                for (int next : list) {
                    inDegrees[next]--;
                    if (inDegrees[next] == 0) {
        ...
        return ans.size() == inDegrees.length;
    }
